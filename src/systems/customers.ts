...
export class CustomerSystem {
  ...
  update(dt:number){
    ...
    for(const c of this.list){
      ...
      else if(c.phase==='browse'){ if(GameState.data.stock>0){ GameState.data.stock--; c.has=true; c.phase='queue'; c.tx=27; c.ty=3 } else { c.phase='queue'; c.tx=27; c.ty=3 } }
      else if(c.phase==='queue'){ this.step(c); if(c.x===27&&c.y===3){ if(c.has) ECON.sellItem(5); c.phase='exit'; c.tx=1; c.ty=9 } }
